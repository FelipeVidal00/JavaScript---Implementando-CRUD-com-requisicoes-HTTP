...
//Constante para validação de Letras maiúsculas, minúsculas e espaços.
const regexConteudo = /^[A-Za-z\s]{10,}$/
//Constante para validação do autoria
const regexAutoria = /^[a-zA-Z]{3,15}$/

//Função para validação Conteudo.
function validarConteudo(conteudo) {
  return regexConteudo.test(conteudo)
}

//Função validação Autoria.
...
async function manipularSubmissaoFormulario(event) {
  ...
  //If de Alerta de Erro para validação do Conteudo.
  if(!validarConteudo(conteudoSemEspacos)) {
    alert("É permitida a inclusão apenas de letras e espaços, com no mínimo 10 caracteres")
    return
  }

  //If de Alerta de Erro para Validação do Autoria.
  if (!validarAutoria(autoriaSemEspacos)) {
    alert("É permitida a inclusçao apenas de letras, de 3 a 15 caracteres, sem espaços")
    return
  }

  if (!validarData(data)) {
    alert("Não é permitido a inclusão de datas futuras, selecione outra data.")
  }
